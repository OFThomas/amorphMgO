...
class matlab:
  ...
  def draw(self):
    ...
    cmd = cmd[:-1] + ",'LineWidth',2)"    # kludge on line width for now
    ...
    self.__call__("title('%s')" % fig.title)
    self.__call__("xlabel('%s')" % fig.xtitle)
    self.__call__("ylabel('%s')" % fig.ytitle)
    ...
    for i in range(fig.nlabels):
      x = fig.labels[i][0]
      y = fig.labels[i][1]
      text = fig.labels[i][2]           # kludge to make labels bigger font
