...
#    self.w.pack(expand=YES,fill=BOTH)
  ...
  # create GL calllist for triangles
  ...
  def reload(self):
    ...
    if self.triframes[0]: self.make_tri_calllist(self.triframes[0])
  ...
  def show(self,ntime):
    ...
    if bonds: self.bonds_augment(bonds)
    if tris: self.make_tri_calllist(tris)
  ...
  def all(self,*list):
	...
	if n == nstart or self.panflag: self.center = compute_center(box)
        ...
        if bonds: self.bonds_augment(bonds)
        if n == nstart and tris: self.make_tri_calllist(tris)
...
#        glEnable(GL_LIGHTING)
      ...
      if not self.clipflag:
        ...
        if self.tridraw: glCallList(self.tcalllist)
      ...
      # draw clipped scene = atoms, bonds, triangles
        ...
        for bond in self.bonddraw:
          ...
          if xmin >= xlo and xmax <= xhi and ymin >= ylo and ymax <= yhi and \
        ...
        fillflag = 0
        if self.tridraw: fillflag = self.vizinfo.tfill[int(self.tridraw[0][1])]
...
#      glEnd()
  ...
  # --------------------------------------------------------------------
  # make new call list for all triangles
  # either use arg as tri list, or self.tridraw if no arg
  # called when tris are loaded via viz() or when tri color is changed

  def make_tri_calllist(self,tris=None):
    if not tris: tris = self.tridraw
    if self.tcalllist: glDeleteLists(self.tcalllist,1)
    self.tcalllist = glGenLists(1)

    fillflag = 0
    if tris: fillflag = self.vizinfo.tfill[int(tris[0][1])]
    
    glNewList(self.tcalllist,GL_COMPILE)
