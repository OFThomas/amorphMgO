...
docstr = """
p = plotview(d,p)       create GUI for viewing plots

  d = Pizza.py object that contains vectors (log, vec)
  p = Pizza.py plotting object (gnu, matlab)
...
class plotview:
  ...
  def __init__(self,source,plot):
    ...
    for i in range(self.nplots):
      ...
      self.plot.xtitle(self.names[0])
      self.plot.ytitle(self.names[i+1])
      self.plot.title(self.names[i+1])
    ...
    for i in range(self.nplots):
      if int(self.checkvars[i].get()) != self.checkold[i]:
        if self.checkvars[i].get():
          ...
          self.plot.select(i+1)
          x,y = self.source.get(self.names[0],self.names[i+1])
          self.plot.plot(x,y)
        else:
