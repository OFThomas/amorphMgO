...
#   trilist = list of tris to return via viz() for all snapshots
#   linelist = list of lines to return via viz() for all snapshots
#   Snap = one snapshot
#     time = time stamp
...
class dump:
  ...
  # --------------------------------------------------------------------
  # sort atoms by ID in all selected timesteps or one timestep
  ...
  # --------------------------------------------------------------------
  # return list of atoms to viz for snapshot with index i
  ...
  def viz(self,i):
    ...
    tris = []
    if self.triflag: tris = self.trilist
    lines = []
  ...
  # --------------------------------------------------------------------
  # return maximum atom type across all selected snapshots

  def maxatomtype(self):
    maxtype = 0
    for snap in self.snaps:
      if not snap.tselect: continue
      if xlo == None or snap.xlo < xlo: xlo = snap.xlo
  ...
  # --------------------------------------------------------------------
  # grab bonds/tris/lines from a bond dump file or data file
  ...
  def extra(self,arg):
    if type(arg) is types.StringType:      # read bonds from bond dump file
      ...
      try:                                  # grab bonds from data object
        ...
        try:                                # grab tris/lines from cdata object
          ...
          raise StandardError,"could not extract from data object"
